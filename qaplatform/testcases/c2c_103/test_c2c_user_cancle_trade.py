...
date: 20180917
email:
content: 为了实时获取可以取消的交易id, 先去做一键匹配，去下单，然后再去取消这笔单子
        所以测试过程中不需要依赖什么数据
other:
'''
import json
...
from qaplatform.testdatas.c2c_103 import data_c2c_user_cancle_trade



##读取配置文件的接口request url  和   request headers
url_login = data_c2c_user_cancle_trade.url_login
token_user = data_c2c_user_cancle_trade.token_user
t_headers_login = data_c2c_user_cancle_trade.t_headers_login
t_headers_logged = data_c2c_user_cancle_trade.t_headers_logged
token_clienttype = data_c2c_user_cancle_trade.token_clienttype

url_buy = data_c2c_user_cancle_trade.url_c2c_user_buy
url_create = data_c2c_user_cancle_trade.url_c2c_user_create_trade
url = data_c2c_user_cancle_trade.url_c2c_user_cancle_trade


class C2C_cancle_trade(unittest.TestCase):
    '''
    法币购买-下买单,然后取消
    '''
    ...
    def setUp(self):
        obj_token = headertoken.PackgingToken(url_login, token_user,
                                              data_c2c_user_cancle_trade.t_headers_login,
                                              data_c2c_user_cancle_trade.token_clienttype)
    ...
    def test_No1031101_buy_BTC_cancle_trade_success(self):
        """UCEX QA：buy BTC, get the order id and create trade, then cancle"""
        logger.logger.debug("doing test " + str(self.id()) + " started")
        # 一键匹配需要的数据
        conf_input_data_buy = data_c2c_user_cancle_trade.ucex_inputdata_buy

        # 下单需要的数据
        conf_input_data_create = data_c2c_user_cancle_trade.ucex_inputdata_create

        #取消交易需要的数据
        conf_input_data = data_c2c_user_cancle_trade.ucex_inputdata[0]
        conf_output_data = data_c2c_user_cancle_trade.ucex_outputdata[0]
        logger.logger.info("token value "+str(self.token_value))

        data_c2c_user_cancle_trade.t_headers_logged["ucex-web-api-token"] = self.token_value
        t_header = data_c2c_user_cancle_trade.t_headers_logged
    ...
    def test_No1031102_cancle_unexist_trade(self):
        """UCEX QA： cancle otc buy trade, but trade id not exist"""
        logger.logger.debug("doing test " + str(self.id()) + " started")

        #取消交易需要的数据
        conf_input_data = data_c2c_user_cancle_trade.ucex_inputdata[1]
        conf_output_data = data_c2c_user_cancle_trade.ucex_outputdata[1]
        logger.logger.info("token value "+str(self.token_value))
